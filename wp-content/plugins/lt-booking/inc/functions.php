...
if (!function_exists('ltb_the_total_grid')) {
	...
	function ltb_the_total_grid( $config ) {
		...
		$out .= '<div class="ltb-grid" data-minutes="'.esc_attr($config['minutes']).'" data-currency="'.esc_attr($config['currency']).'" data-currency-pos="'.esc_attr($config['currency-pos']).'" data-plan-placeholder="'.esc_attr($config['total-placeholder-2']).'" data-time-placeholder="'.esc_attr($config['total-placeholder-4']).'">';
			...
			foreach ( $items as $class => $item) {
				...
				$out .= '<div class="ltb-grid-item"><div class="ltb-grid-content '.esc_attr($class).'">';
					$out .= '
...
if ( !function_exists('ltb_make_booking') ) {
	...
	function ltb_make_booking() {
		...
		if ( !empty($booking['ltb-services']) ) {
			...
			$booking['ltb-services'] = explode(',', $booking['ltb-services']);
		}
			else {
		...
		fw_set_db_post_option($post_id, 'booking-date', date('Y-m-d H:i', $booking['ltb-booking-date']));
		...
		echo '<h5 class="header">'.wp_kses_post(ltb_header_parse($config['submit-header'])).'</h5>';
...
/**
 * Parse fields for email template
 */
	...
	function ltb_parse_email_fields( $post_id, $config, $string = '' ) {
		...
		if ( !empty($string) ) {
			...
			$replace = array(
				...
				'{order_info}'		=>	ltb_order_info( $config, $item ),
...
if ( !function_exists('ltb_order_info') ) {
	...
	function ltb_order_info( $config, $item ) {

		$string = $config['order-info'];
		...
		$replace = array(
			...
			'{booking-date}'	=>	date('Y-m-d H:i', strtotime($item['booking-date'])),
		);	
...
if ( !function_exists('ltb_google_calendar_link') ) {
	...
	function ltb_google_calendar_link( $config, $item ) {
		...
		$date = new DateTime(date("Y-m-d H:i", $item['booking-date']));
