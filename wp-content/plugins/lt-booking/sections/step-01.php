	...
	function lt_booking_step_01_shortcode( $atts ) {
		...
		if ( !empty($config['cars']) ) {
			...
			if ( !empty($cars) ) {
				...
				$car_types = $tariffs = $pagination = array();
				foreach ( $cars as $car ) {
				...
				$out .= '<div class="swiper-wrapper" data-cars="'.filter_var( json_encode($car_types), FILTER_SANITIZE_SPECIAL_CHARS ).'" data-tariffs="'.filter_var( json_encode($tariffs), FILTER_SANITIZE_SPECIAL_CHARS ).'" data-pagination=\''. filter_var( json_encode($pagination), FILTER_SANITIZE_SPECIAL_CHARS ) .'\'>';
